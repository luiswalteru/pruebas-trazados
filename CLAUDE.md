...
The ilustrated assets that accompany each trazado (`base.svg` background illustration + `guia.svg` dashed guide, and optionally character art / audio) are authored upstream and shipped by the content pipeline. This tool **consumes** `base.svg` + `guia.svg` as inputs to drive the drawing experience (visual guide + skeleton for snap), but does **not** re-emit them. Note: the uploaded `base.svg` (illustration) and the `base.svg` this tool emits (animation template) share a filename but are different files — the former is backdrop only, the latter contains the `<path class="svgPath">` the reader animates.
...
- **`base.svg`** (uploaded) — the ilustration (coloured letter body, directional arrows, order number, decorative characters). Shown as a visual background, never inspected programmatically. Unrelated to the `base.svg` this tool *emits* (animation template).
- **`guia.svg`** — the dashed guide showing where the stroke should go. Shown on top of the uploaded `base.svg` as the visible trace guide, **and** rasterized internally to extract the skeleton the drawn strokes will snap to.
...
- `adjustStrokeToGuide(points)` decides which adjuster to run based on the guide mode:
  - **`skeleton`** (polyline extracted from `guia.svg`): `projectStrokeOnGuide(points, guide)` — snaps the first point to the nearest polyline endpoint if one is within 20 px, projects every subsequent point onto the nearest polyline segment with a **direction-aware lateral bias** (see below), snaps the last point to an endpoint if close, and runs two passes of neighbour-averaging to clean up sample-to-sample segment-switch jitter.
  - **`fallback`** (raster mask from `guia.svg`, polyline extraction failed): `centerStrokePoints(points, maskInfo)` — the legacy iterative distance-transform pull.
...
At finalize time (`handleFinalize`): for each already-adjusted stroke, `resample` to `dotCount` equidistant points, mark corners (angle delta > π/4), `toFixed(3)` coords. For the `dragger` we subtract `(10, 10)` from the first resampled point before `toFixed(0)` — the reader renders `#fixedDot` as a 20×20 px `div` positioned via `transform: translate(draggerX px, draggerY px)`, which anchors the div's top-left, not its centre; without the −10 offset the blue dot sits 10 px down-right of the stroke's start when the auto-animation finishes. The canonical reference `ejemplo/trazado-letra-a/data.json` follows the same rule: `dragger = [31, 148]` ≈ `firstCoord [40.944, 160.184] − (10, 10)`. `strokePaths` are built separately with a lighter `smooth(_, 2)` for `base.svg`. **The drawer no longer emits `skeletonPaths`** (no `letter-dotted.svg` is produced any more).
...
### Skeleton extraction from `guia.svg`

`extractGuideFromSvg(guiaSvg, width, height)` is a thin wrapper around `extractGuideMaskFromImage` with `mode: 'any-opaque'`:

1. Rasterize the SVG on a **transparent** canvas at 2× resolution (object-fit: contain).
2. Binarize: any pixel with `alpha >= 64` counts as guide (colour doesn't matter — `guia.svg` typically has a single dark fill for all dashes).
...
If extraction returns fewer than 3 centroids or no edges → fallback to `buildMaskFromImage(guiaSvg, ...)` (raster distance transform). If that also fails → `maskMode = 'none'`, stroke saved raw.

The "Ver guía" toggle in the drawer overlays the detected guide (cyan edges, dark-teal dots, orange endpoints) — useful when the skeleton has gaps or spurs in a new `guia.svg` design. Thresholds to tune live in `guideExtractor.js`: `minAlpha`, `minArea`, `minComponentRatio`, `closePasses`, `renderScale`.

The drawer does **not** render a separate dashed overlay on top of the canvas — the uploaded `guia.svg` itself is the visible guide, so "what the user sees while drawing" is exactly what they uploaded.
...
- `<path id="path{i+1}" class="svgPath" stroke="#f04e23" fill="none" stroke-width="S" d="..."/>` per user stroke. `d` is `strokePaths[i].d` from `ManualPathDrawer.handleFinalize` (`M x,y L x,y ...` over `smooth(pts, 2)`).
- `stroke-width="S"` baked in from `effStroke` (= `animationPathStroke` in `data.json`, fixed 16 unless the user overrides).
- `stroke="#f04e23"` and `fill="none"` inline as presentation-attribute fallbacks. The reader's CSS overrides both; the inline attrs are a safety net so the path isn't rendered black-filled or black-stroked if CSS fails / hasn't applied yet.
- `<circle id="circle" cx cy r fill="blue">` at the first point of the first stroke, `r = Math.ceil(stroke / 1.4)`. `fill="blue"` inline matches the reference `base.svg` and keeps the marker visible if the reader's CSS hasn't applied yet.
...
Canvas size is **not** auto-computed — it's whatever the user configures in Step 2 (default 380 × 340).

- `computeLetterParams(letter, type, canvasW)` returns `{ dotSize, animationPathStroke }`. **`animationPathStroke` is a constant 16** for every letter and both types — it matches `stroke-width="16"` in the canonical reference `ejemplo/trazado-letra-a/base.svg` / `data.json`. The `base.svg` generator bakes this into each `<path>`, so keeping it fixed ensures the animated stroke weight is consistent with the reader's existing catalogue. `dotSize` still varies: canvas-width buckets for ligada + letter-specific overrides for `e`, `i`, `k`, `m`, `n`, `u`, `p`, and `canvasW > 240` threshold for mayusculas.

**Override convention** (important for `GeneratorPage` inputs): `0 = auto-compute, >0 = user forces this value`. Canvas w/h don't have this convention — any user value is used directly. The `strokeWidth` input overrides the fixed 16 if the user sets it `> 0`.

**Uploaded-SVG dimensions**: `handleSvgUpload` calls `parseSvgDims(dataUrl)` (local helper) to parse intrinsic `width`/`height` (or `viewBox` as fallback) from the SVG. If the result differs from `canvasWidth`×`canvasHeight`, a `console.warn` is emitted. The canvas is **not** auto-overridden — the canvas feeds `computeLetterParams` and the drawer's display size, so silently resizing it would change `dotSize` / `animationPathStroke` and the visual scale of the drawing surface. The warning is advisory only: the user must reconcile the viewBoxes manually in Step 2 if the reader needs the generated animation template's viewBox to match the uploaded `base.svg` / `guia.svg`.
...
The wizard's full state is mirrored into `window.__generatorState` (not Context, not Redux) via an always-running `useEffect` inside `GeneratorPage.jsx`, and the current step is kept in the URL as `?step=N`. The `images` state in particular has shape `{ [letter]: { base: dataURL, guia: dataURL } }` — preserve that shape when touching Step 1 or persistence. Preview data is passed to `/preview` through `window.__trazadoPreview` and includes `baseSvg` + `guiaSvg` so the preview reproduces the same visual backdrop the user drew on. `baseSvg` in the preview payload is the **uploaded illustration**, not the generated animation template — PreviewPage reconstructs the stroke animation from `dataJson.dotList` and doesn't consume the emitted template, so there's no collision on the payload key.
...
- `ejemplo/trazado-letra-a/base.svg` and `ejemplo/trazado-letra-a/guia.svg` are the **canonical reference inputs**: exactly what the user is expected to upload in Step 1. The rest of that folder (`character.png`, `fondo.png`, audio files, `letter-fill.svg`, `letter-outline.svg`, `letter-dotted.svg`, `thum.png`) is a **historical** output bundle from an earlier generator version and does **not** match current export — it's kept only for reference.
